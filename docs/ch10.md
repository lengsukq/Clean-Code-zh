...
**Classes should be small** and follow the Single Responsibility Principle, having one reason to change.
> **类应该小**并遵循单一职责原则，只有一个变更原因。

**High cohesion** means class methods and variables work together as a logical whole.
> **高内聚**意味着类的方法和变量作为逻辑整体协同工作。

**Classes should be organized for change** following the Open-Closed Principle: open for extension, closed for modification.
> **类应该为变更而组织**，遵循开闭原则：对扩展开放，对修改关闭。

## 小结

**The Single Responsibility Principle (SRP)** is key to clean class design and maintainability.
> **单一职责原则(SRP)**是整洁类设计和可维护性的关键。

**Maintaining cohesion** often results in many small, focused classes rather than a few large ones.
> **保持内聚**通常导致许多小的、专注的类，而不是少数大的类。

**Dependency Inversion Principle** helps isolate classes from change by depending on abstractions, not concrete details.
> **依赖倒置原则**通过依赖抽象而非具体细节来帮助类隔离变更。
...
> 对于函数，我们通过计算物理行数来衡量大小。对于类，我们使用不同的衡量标准。我们计算职责。1
...
> 单一职责原则(SRP)2规定类或模块应该有且仅有一个变更原因。这个原则既给了我们职责的定义，也给了类大小的指导原则。类应该有一个职责——一个变更原因。
...
> 类应该有少量的实例变量。类的每个方法应该操作这些变量中的一个或多个。一般来说，一个方法操作的变量越多，该方法与类的内聚性就越强。每个变量都被每个方法使用的类是最大内聚的。
