...
**Systems**, like cities, must be designed with proper **separation of concerns** and **levels of abstraction**. Just as cities have specialized teams managing water, power, traffic, and infrastructure, software systems need modularized components that work together effectively. This chapter explores how to maintain **clean code at the system level**, focusing on critical principles: **separating construction from use**, leveraging **Dependency Injection (DI)** and **Inversion of Control (IoC)**, and applying **aspect-oriented programming (AOP)** to handle **cross-cutting concerns**. The key insight is that systems can and should **grow incrementally**, evolving from simple to complex architectures through **test-driven development** and **refactoring**, rather than requiring extensive Big Design Up Front (BDUF). By writing **domain logic as Plain Old Java Objects (POJOs)** and decoupling architectural concerns through aspects, we can create systems that are **modular, testable, and adaptable** to change—systems that optimize decision-making, use standards wisely, and express domain concepts through **Domain-Specific Languages (DSLs)**.

> **系统**就像城市一样，必须通过适当的**关注点分离**和**抽象层次**来设计。正如城市有专门的团队管理水、电力、交通和基础设施，软件系统也需要模块化的组件有效地协同工作。本章探讨如何在**系统层面保持整洁的代码**，重点关注几个关键原则：**将构造与使用分离**、利用**依赖注入（DI）**和**控制反转（IoC）**，以及应用**面向切面编程（AOP）**来处理**横切关注点**。关键的见解是系统可以而且应该**递增式增长**，通过**测试驱动开发**和**重构**从简单演化到复杂的架构，而不是需要大量的预先大设计（BDUF）。通过将**领域逻辑编写为简单老式 Java 对象（POJOs）**，并通过切面解耦架构关注点，我们可以创建**模块化、可测试和可适应变化**的系统——这些系统能够优化决策制定、明智地使用标准，并通过**领域特定语言（DSLs）**表达领域概念。

## 小结 Chapter Overview  

This chapter addresses the challenge of building **clean systems** by examining several fundamental concepts: First, we explore the importance of **separating the startup process** (object construction and dependency wiring) from runtime logic, using techniques like **Separation of Main**, **Factories**, and **Dependency Injection**. Second, we confront the myth that systems must be "right the first time," demonstrating instead that architectures can **scale incrementally** if we maintain proper separation of concerns—a lesson illustrated through the evolution from problematic EJB2 to cleaner approaches using **Spring**, **JBoss AOP**, and **AspectJ**. Third, we examine how **cross-cutting concerns** like persistence, transactions, and security can be handled through **aspect-like mechanisms** without invasively coupling architectural details to business logic. Finally, we emphasize that **optimal system architecture** consists of modularized domains implemented with POJOs, integrated with minimally invasive aspects, enabling **test-driven development** at the system level, **decentralized decision-making**, judicious use of **standards**, and clear expression through **Domain-Specific Languages**.

> 本章通过考察几个基本概念来解决构建**整洁系统**的挑战：首先，我们探讨**分离启动过程**（对象构造和依赖关系连接）与运行时逻辑的重要性，使用诸如**主程序分离**、**工厂模式**和**依赖注入**等技术。其次，我们直面系统必须"第一次就做对"的神话，转而展示如果我们保持适当的关注点分离，架构可以**递增式扩展**——这个教训通过从有问题的 EJB2 演变到使用 **Spring**、**JBoss AOP** 和 **AspectJ** 的更整洁方法得以说明。第三，我们考察如何通过**类切面机制**处理诸如持久化、事务和安全性等**横切关注点**，而无需侵入式地将架构细节耦合到业务逻辑。最后，我们强调**最优系统架构**由用 POJOs 实现的模块化领域组成，与最少侵入性的切面集成，在系统级别实现**测试驱动开发**、**分散式决策**、明智使用**标准**，以及通过**领域特定语言**清晰表达。

---

## HOW WOULD YOU BUILD A CITY? 如何建造城市？
...
> 你能自己管理所有细节吗？大概不能。即便是管理一个现有的城市，对一个人来说也过于繁重。然而，城市能够运转（大多数时候）。城市之所以能运转，是因为有一些团队的人管理城市的特定部分，如水系统、电力系统、交通、执法、建筑规范等等。其中一些人负责全局，而另一些人则专注于细节。
...
> 城市能够运转，还因为它们演化出了恰当的抽象和模块化层次，使得个人和他们所管理的"组件"即使在不了解全局的情况下也能有效工作。
...
> 虽然软件团队通常也这样组织，但他们工作的系统往往没有相同的关注点分离和抽象层次。整洁代码帮助我们在较低的抽象层次上实现这一点。在本章中，让我们考虑如何在更高的抽象层次——系统层面——保持整洁。
...
> 首先，要考虑到构造是一个与使用非常不同的过程。在我写这段话的时候，我能从芝加哥的窗外看到一家正在建设中的新酒店。今天它还是一个光秃秃的混凝土盒子，外面固定着建筑起重机和电梯。在那里忙碌的人都戴着安全帽，穿着工作服。大约一年之后，酒店将会完工。起重机和电梯将消失。建筑物会很整洁，被玻璃幕墙和漂亮的油漆包裹。在那里工作和居住的人看起来也会大不相同。

Software systems should separate the startup process, when the application objects are constructed and the dependencies are "wired" together, from the runtime logic that takes over after startup.

> 软件系统应该将启动过程（此时应用对象被构造，依赖关系被"连接"在一起）与启动后接管的运行时逻辑分离开来。
...
> 启动过程是任何应用都必须处理的一个关注点。这是我们在本章中要考察的第一个关注点。关注点分离是我们这门手艺中最古老、最重要的设计技术之一。
...
> 不幸的是，大多数应用程序没有分离这个关注点。启动过程的代码是临时的，并且与运行时逻辑混在一起。这是一个典型的例子：
...
> 这是**延迟初始化/求值**惯用法，它有几个优点。除非我们真正使用对象，否则不会产生构造的开销，因此我们的启动时间可以更快。我们还确保永远不会返回 null。
...
> 然而，我们现在对 MyServiceImpl 及其构造函数所需的一切（我已省略）有了硬编码的依赖。即使我们在运行时从不实际使用这种类型的对象，也无法在不解决这些依赖关系的情况下编译！
...
> 测试可能会是个问题。如果 MyServiceImpl 是一个重量级对象，我们需要确保在单元测试期间调用此方法之前，将适当的**测试替身**[1] 或**模拟对象**分配给 service 字段。由于我们将构造逻辑与正常的运行时处理混合在一起，我们应该测试所有执行路径（例如，null 测试及其代码块）。拥有这两个职责意味着该方法做了不止一件事，所以我们在小范围内违反了**单一职责原则**。
...
> 也许最糟糕的是，我们不知道在所有情况下 MyServiceImpl 是否都是正确的对象。我在注释中暗示了这一点。为什么带有此方法的类必须知道全局上下文？我们真的能知道在这里使用哪个对象是正确的吗？一种类型是否可能适用于所有可能的上下文？
...
> 当然，一次**延迟初始化**并不是一个严重的问题。然而，应用程序中通常有许多这样的小设置惯用法实例。因此，全局设置策略（如果有的话）分散在整个应用程序中，模块化程度很低，而且经常有明显的重复。
...
> 如果我们勤勉地构建结构良好且健壮的系统，就不应该让这些方便的小惯用法导致模块化的崩溃。对象构造和连接的启动过程也不例外。我们应该将这个过程与正常的运行时逻辑分开模块化，并且应该确保我们有一个全局、一致的策略来解决主要的依赖关系。
...
> 将构造与使用分离的一种方法是简单地将构造的所有方面移到 main 或由 main 调用的模块中，并设计系统的其余部分，假设所有对象都已被适当地构造和连接。（见图 11-1。）
...
> 控制流很容易理解。main 函数构建系统所需的对象，然后将它们传递给应用程序，应用程序只是使用它们。注意跨越 main 和应用程序之间屏障的依赖箭头的方向。它们都指向一个方向，远离 main。这意味着应用程序不知道 main 或构造过程。它只是期望一切都已正确构建。
...
> 当然，有时我们需要让应用程序负责何时创建对象。例如，在订单处理系统中，应用程序必须创建 LineItem 实例以添加到 Order 中。在这种情况下，我们可以使用**抽象工厂**[2] 模式让应用程序控制何时构建 LineItems，但将构造的细节与应用程序代码分开。（见图 11-2。）
...
> 再次注意，所有依赖关系都从 main 指向 OrderProcessing 应用程序。这意味着应用程序与如何构建 LineItem 的细节解耦。该能力保留在 LineItemFactoryImplementation 中，它位于分界线的 main 一侧。然而，应用程序完全控制何时构建 LineItem 实例，甚至可以提供特定于应用程序的构造函数参数。
...
> 将构造与使用分离的一个强大机制是**依赖注入（DI）**，即将**控制反转（IoC）**应用于依赖管理[3]。控制反转将次要职责从对象转移到专门用于此目的的其他对象，从而支持**单一职责原则**。在依赖管理的上下文中，对象不应该负责实例化依赖项本身。相反，它应该将这个职责传递给另一个"权威"机制，从而反转控制。由于设置是一个全局关注点，这个权威机制通常是"main"例程或专用容器。
...
> JNDI 查找是 DI 的"部分"实现，其中对象向目录服务器请求提供与特定名称匹配的"服务"。
...
> 调用对象不控制实际返回什么类型的对象（当然，只要它实现了适当的接口），但调用对象仍然主动解析依赖关系。
...
> 真正的依赖注入更进一步。类不采取直接步骤来解析其依赖关系；它是完全被动的。相反，它提供 setter 方法或构造函数参数（或两者），用于注入依赖项。在构造过程中，DI 容器实例化所需的对象（通常按需），并使用提供的构造函数参数或 setter 方法来连接依赖项。实际使用哪些依赖对象是通过配置文件或在专用构造模块中以编程方式指定的。
...
> Spring 框架为 Java 提供了最著名的 DI 容器[4]。你在 XML 配置文件中定义要连接哪些对象，然后在 Java 代码中按名称请求特定对象。我们很快就会看到一个例子。
...
> 但是**延迟初始化**的优点呢？这个惯用法在 DI 中有时仍然有用。首先，大多数 DI 容器在需要之前不会构造对象。其次，许多这些容器提供了调用工厂或构造代理的机制，可用于**延迟求值**和类似的优化[5]。
...
> 城市从城镇发展而来，城镇从定居点发展而来。起初道路狭窄，几乎不存在，然后被铺设，随着时间的推移而加宽。小建筑和空地被更大的建筑填满，其中一些最终会被摩天大楼取代。
...
> 起初没有电力、水、污水处理和互联网（天哪！）等服务。随着人口和建筑密度的增加，这些服务也被添加进来。
...
> 这种增长并非没有痛苦。你有多少次开车时被道路"改善"项目堵得一动不动，并问自己："为什么他们第一次不把它建得够宽呢！？"
...
> 但这别无他法。谁能证明在一个预期增长的小镇中间修建六车道高速公路的费用是合理的？谁会想要这样一条路穿过他们的城镇？

It is a myth that we can get systems "right the first time." Instead, we should implement only today's stories, then refactor and expand the system to implement new stories tomorrow. This is the essence of iterative and incremental agility. Test-driven development, refactoring, and the clean code they produce make this work at the code level.

> 我们可以"第一次就把系统做对"是一个神话。相反，我们应该只实现今天的故事，然后重构和扩展系统以实现明天的新故事。这是**迭代和增量敏捷性**的本质。**测试驱动开发**、**重构**以及它们产生的整洁代码使这在代码层面发挥作用。
...
> 但在系统层面呢？系统架构不需要预先规划吗？当然，它不能从简单到复杂地递增增长，不是吗？
...
> 与物理系统相比，软件系统是独特的。如果我们保持适当的关注点分离，它们的架构可以递增地增长。
...
> 正如我们将看到的，软件系统的短暂性使这成为可能。让我们首先考虑一个没有充分分离关注点的架构的反例。
...
> 原始的 EJB1 和 EJB2 架构没有适当地分离关注点，从而对有机增长施加了不必要的障碍。考虑持久化 Bank 类的实体 Bean。实体 bean 是关系数据的内存表示，换句话说，是表行。
...
> 首先，你必须定义一个本地（进程内）或远程（单独的 JVM）接口，客户端将使用它。清单 11-1 显示了一个可能的本地接口：
...
> 我展示了 Bank 地址的几个属性和银行拥有的账户集合，每个账户的数据都由单独的 Account EJB 处理。清单 11-2 显示了 Bank bean 的相应实现类。
...
> 我没有显示相应的 LocalHome 接口，它本质上是用于创建对象的工厂，也没有显示你可能添加的任何可能的 Bank 查找（查询）方法。
...
> 最后，你必须编写一个或多个 XML 部署描述符，这些描述符指定对象关系映射到持久化存储的细节、所需的事务行为、安全约束等等。
...
> 业务逻辑与 EJB2 应用程序"容器"紧密耦合。你必须继承容器类型，并且必须提供容器所需的许多生命周期方法。
...
> 由于与重量级容器的这种耦合，独立的单元测试很困难。有必要模拟出容器，这很难，或者浪费大量时间将 EJBs 和测试部署到真实服务器。由于紧密耦合，在 EJB2 架构之外重用实际上是不可能的。
...
> 最后，甚至面向对象编程也被破坏了。一个 bean 不能从另一个 bean 继承。注意添加新账户的逻辑。在 EJB2 bean 中定义本质上是没有行为的"结构体"的"数据传输对象"（DTOs）是很常见的。这通常导致持有本质上相同数据的冗余类型，并且需要样板代码将数据从一个对象复制到另一个对象。
...
> EJB2 架构在某些领域接近真正的关注点分离。例如，所需的事务、安全和一些持久化行为在部署描述符中声明，独立于源代码。
...
> 请注意，像持久化这样的关注点往往会跨越领域的自然对象边界。你希望使用通常相同的策略来持久化所有对象，例如，使用特定的 **DBMS**[6] 而不是平面文件，遵循表和列的某些命名约定，使用一致的事务语义等等。
...
> 原则上，你可以以模块化、封装的方式推理你的持久化策略。然而，在实践中，你必须将实现持久化策略的本质上相同的代码分散到许多对象中。我们使用术语**横切关注点**来描述这样的关注点。同样，持久化框架可能是模块化的，而我们的领域逻辑孤立来看也可能是模块化的。问题在于这些领域的细粒度交叉。
...
> 事实上，EJB 架构处理持久化、安全和事务的方式"预示了"**面向切面编程（AOP）**[7]，这是一种恢复横切关注点模块化的通用方法。
...
> 在 AOP 中，称为**切面**的模块化构造指定系统中的哪些点应该以某种一致的方式修改其行为以支持特定的关注点。这种规范使用简洁的声明性或编程机制来完成。
...
> 以持久化为例，你将声明哪些对象和属性（或其模式）应该被持久化，然后将持久化任务委托给你的持久化框架。行为修改由 AOP 框架以**非侵入性**[8] 的方式对目标代码进行。让我们看看 Java 中的三种切面或类切面机制。
...
> Java 代理适用于简单的情况，例如包装单个对象或类中的方法调用。然而，JDK 中提供的动态代理仅适用于接口。要代理类，必须使用字节码操作库，例如 CGLIB、ASM 或 Javassist[9]。
...
> 清单 11-3 显示了 JDK 代理的骨架，为我们的 Bank 应用程序提供持久化支持，仅涵盖获取和设置账户列表的方法。
...
> 我们定义了一个接口 Bank，它将被代理包装，以及一个**简单老式 Java 对象（POJO）** BankImpl，它实现业务逻辑。（我们稍后将重新讨论 POJOs。）
...
> Proxy API 需要一个 InvocationHandler 对象，它调用它来实现对代理进行的任何 Bank 方法调用。我们的 BankProxyHandler 使用 Java 反射 API 将通用方法调用映射到 BankImpl 中的相应方法等等。
...
> 这里有很多代码，即使对于这个简单的情况，它也相对复杂[10]。使用字节操作库之一也同样具有挑战性。这种代码"量"
...
> 和复杂性是代理的两个缺点。它们使创建整洁的代码变得困难！此外，代理不提供用于指定系统范围执行"点"的机制，这对于真正的 AOP 解决方案是必需的[11]。
...
> 幸运的是，大多数代理样板可以由工具自动处理。代理在几个 Java 框架中内部使用，例如 Spring AOP 和 JBoss AOP，以纯 Java 实现切面[12]。在 Spring 中，你将业务逻辑编写为简单老式 Java 对象。POJOs 纯粹专注于它们的领域。它们不依赖于企业框架（或任何其他领域）。因此，它们在概念上更简单，更容易进行测试驱动。相对的简单性使得更容易确保你正确实现了相应的用户故事，并为未来的故事维护和演化代码。
...
> 你使用声明性配置文件或 API 整合所需的应用程序基础设施，包括持久化、事务、安全、缓存、故障转移等横切关注点。在许多情况下，你实际上是在指定 Spring 或 JBoss 库切面，框架透明地处理使用 Java 代理或字节码库的机制。这些声明驱动依赖注入（DI）容器，该容器实例化主要对象并按需将它们连接在一起。

Listing 11-4 shows a typical fragment of a Spring V2.5 configuration file, app.xml13:

> 清单 11-4 显示了 Spring V2.5 配置文件 app.xml 的典型片段[13]：
...
> 每个"bean"就像一个嵌套的"俄罗斯套娃"的一部分，Bank 的领域对象被数据访问对象（DAO）代理（包装），而 DAO 本身被 JDBC 驱动程序数据源代理。（见图 11-3。）
...
> 客户端认为它正在 Bank 对象上调用 getAccounts()，但它实际上正在与一组嵌套的**装饰器**[14] 对象的最外层对话，这些对象扩展了 Bank POJO 的基本行为。我们可以为事务、缓存等添加其他装饰器。
...
> 在应用程序中，需要几行代码来向 DI 容器请求系统中的顶级对象，如 XML 文件中所指定的。
...
> 因为只需要很少的 Spring 特定的 Java 代码行，应用程序几乎完全与 Spring 解耦，消除了像 EJB2 这样的系统的所有紧密耦合问题。
...
> 虽然 XML 可能冗长且难以阅读[15]，但这些配置文件中指定的"策略"比隐藏并自动创建的复杂代理和切面逻辑更简单。这种类型的架构如此引人注目，以至于像 Spring 这样的框架导致了 EJB 标准版本 3 的完全改革。EJB3
...
> 在很大程度上遵循 Spring 模型，使用 XML 配置文件和/或 Java 5 注解声明性地支持横切关注点。

Listing 11-5 shows our Bank object rewritten in EJB316.

> 清单 11-5 显示了我们在 EJB3 中重写的 Bank 对象[16]。
...
> 这段代码比原始的 EJB2 代码整洁得多。一些实体细节仍然在这里，包含在注解中。然而，因为没有任何信息在注解之外，代码是整洁的、清晰的，因此易于测试驱动、维护等等。
...
> 如果需要，注解中的部分或全部持久化信息可以移到 XML 部署描述符中，留下真正纯粹的 POJO。如果持久化映射细节不会频繁更改，许多团队可能会选择保留注解，但与 EJB2 的侵入性相比，有害的缺点要少得多。
...
> 最后，通过切面分离关注点的最全功能工具是 AspectJ 语言[17]，它是 Java 的扩展，为切面作为模块化构造提供"一流"支持。Spring AOP 和 JBoss AOP 提供的纯 Java 方法足以满足切面最有用的 80-90% 的情况。然而，AspectJ 为分离关注点提供了非常丰富和强大的工具集。AspectJ 的缺点是需要采用几种新工具并学习新的语言构造和使用惯用法。
...
> 采用问题已被最近引入的 AspectJ 的"注解形式"部分缓解，其中 Java 5 注解用于使用纯 Java 代码定义切面。此外，Spring 框架具有许多功能，使得对于 AspectJ 经验有限的团队来说，整合基于注解的切面变得更加容易。

A full discussion of AspectJ is beyond the scope of this book. See [AspectJ], [Colyer], and [Spring] for more information.

> AspectJ 的完整讨论超出了本书的范围。有关更多信息，请参见 [AspectJ]、[Colyer] 和 [Spring]。
...
> 通过类切面方法分离关注点的力量怎么强调都不为过。如果你可以使用 POJOs 编写应用程序的领域逻辑，在代码级别与任何架构关注点解耦，那么就有可能真正测试驱动你的架构。你可以根据需要通过按需采用新技术从简单演化到复杂。没有必要进行**预先大设计**[18]（BDUF）。事实上，BDUF 甚至是有害的，因为它抑制了适应变化，因为心理上抵制放弃先前的努力，以及架构选择影响后续设计思考的方式。
...
> 建筑架构师必须进行 BDUF，因为一旦施工进行得很顺利，就不可能对大型物理结构进行根本性的架构变更[19]。虽然软件有其自己的物理学[20]，但如果软件结构有效地分离其关注点，那么进行根本性的变更在经济上是可行的。
...
> 这意味着我们可以从一个"天真简单"但很好解耦的架构开始软件项目，快速交付可工作的用户故事，然后随着规模的扩大添加更多的基础设施。世界上一些最大的网站已经实现了非常高的可用性和性能，使用复杂的数据缓存、安全、虚拟化等等，所有这些都是高效且灵活地完成的，因为最小耦合的设计在每个抽象和范围级别都是适当简单的。
...
> 当然，这并不意味着我们"毫无方向"地进入一个项目。我们对项目的总体范围、目标和进度以及最终系统的总体结构有一些期望。然而，我们必须保持根据不断变化的情况改变方向的能力。
...
> 早期的 EJB 架构只是许多过度设计且妥协关注点分离的著名 API 之一。即使是设计良好的 API，当它们并不真正需要时也可能是过度的。一个好的 API 大部分时间应该在很大程度上从视野中消失，因此团队将其大部分创造性努力集中在正在实现的用户故事上。如果不是，那么架构约束将抑制向客户有效交付最优价值。
...
> **最优系统架构由模块化的关注领域组成，每个领域都使用简单老式 Java（或其他）对象实现。不同的领域通过最少侵入性的切面或类切面工具集成在一起。这种架构可以像代码一样进行测试驱动。**
...
> 模块化和关注点分离使分散式管理和决策成为可能。在一个足够大的系统中，无论是城市还是软件项目，没有一个人可以做出所有决定。
...
> 我们都知道最好将责任交给最合格的人。我们经常忘记，最好将决定推迟到最后可能的时刻。这不是懒惰或不负责任；它让我们用最好的信息做出明智的选择。**过早的决定是在次优知识下做出的决定**。如果我们过早决定，我们将得到更少的客户反馈、对项目的思考以及我们实施选择的经验。

The agility provided by a POJO system with modularized concerns allows us to make optimal, just-in-time decisions, based on the most recent knowledge. The complexity of these decisions is also reduced.

> 具有模块化关注点的 POJO 系统提供的敏捷性使我们能够基于最新知识做出**最优的、适时的决策**。这些决定的复杂性也降低了。

## USE STANDARDS WISELY, WHEN THEY ADD DEMONSTRABLE VALUE 明智地使用标准，当它们增加可证明的价值时
...
> 建筑施工是一个奇迹，因为新建筑的建设速度（即使在隆冬时节）以及当今技术可能实现的非凡设计。建筑是一个成熟的行业，拥有在数个世纪的压力下演化出来的高度优化的部件、方法和标准。
...
> 许多团队使用 EJB2 架构是因为它是一个标准，即使更轻量级和更直接的设计就已经足够了。我见过团队痴迷于各种被大肆宣传的标准，而失去了为客户实现价值的焦点。

Standards make it easier to reuse ideas and components, recruit people with relevant experience, encapsulate good ideas, and wire components together. However, the process of creating standards can sometimes take too long for industry to wait, and some standards lose touch with the real needs of the adopters they are intended to serve.

> **标准使重用思想和组件、招聘有相关经验的人员、封装好的思想以及将组件连接在一起变得更容易。**然而，创建标准的过程有时对行业来说可能太长而无法等待，而且一些标准与它们打算服务的采用者的真实需求脱节。
...
> 建筑施工，像大多数领域一样，已经发展出一种丰富的语言，具有词汇、习语和模式[21]，清晰而简洁地传达基本信息。在软件中，最近对创建**领域特定语言（DSLs）**[22] 的兴趣重新燃起，这些是独立的小型脚本语言或标准语言中的 API，允许代码被编写成领域专家可能编写的结构化散文形式。
...
> 一个好的 DSL 最小化了领域概念与实现它的代码之间的"**沟通鸿沟**"，正如敏捷实践优化团队内部以及与项目利益相关者的沟通一样。如果你使用领域专家使用的相同语言来实现领域逻辑，那么错误地将领域转换为实现的风险就会降低。
...
> DSLs 在有效使用时，将抽象级别提升到代码习语和设计模式之上。它们允许开发人员在适当的抽象级别上揭示代码的意图。

Domain-Specific Languages allow all levels of abstraction and all domains in the application to be expressed as POJOs, from high-level policy to low-level details.

> **领域特定语言允许应用程序中的所有抽象级别和所有领域都表达为 POJOs，从高级策略到低级细节。**
...
> 系统也必须是整洁的。**侵入性架构**压倒了领域逻辑并影响了敏捷性。当领域逻辑被掩盖时，质量会受到影响，因为错误更容易隐藏，而故事变得更难实现。如果敏捷性受到损害，生产力就会受到影响，TDD 的好处就会丧失。
...
> 在所有抽象级别上，**意图应该是清晰的**。只有当你编写 POJOs 并使用类切面机制以非侵入性的方式整合其他实现关注点时，这才会发生。

Whether you are designing systems or individual modules, never forget to use the simplest thing that can possibly work.

> 无论你是设计系统还是单个模块，**永远不要忘记使用可能工作的最简单的东西**。
